# dbbackup233 Docs

`dbbackup233` is an agent-first backup and restore CLI for game operations.

Core docs:

- [Technical Architecture](technical-architecture.md)
- [Provider Matrix](provider-matrix.md)
- [Restore Guide](restore.md)
- [Agent-First Product](agent-first.md)
- [Operator Runbook](operator-runbook.md)
- [Benchmark](benchmark.md)
- [HTTP Reporting](http-reporting.md)
- [Upload Provider](upload-provider.md)

Specs:

- [Backup Center Spec](specs/backup-center-spec.md)
- [Testing Spec](specs/testing-spec.md)
- [Implementation Plan](specs/implementation-plan.md)
